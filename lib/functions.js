...
const getGroupAdmins = (participants) => {
    const admins = [];
    if (!participants) return admins; // Safety check
    for (let participant of participants) {
        // --- FIX FOR BAILEYS 6.7.x ---
        // Purana code: participant.admin !== null
 ...
 */
const h2k = (num) => {
    if (typeof num !== 'number' || isNaN(num)) return num; // Safety check
    const tiers = ['', 'K', 'M', 'B', 'T'];
